...
export default function Post({ mode }) {
  ...
  useEffect(() => {
    ...
    setPostData({title: "Test Post", body: testMD});
  }, []);

  // TO DO - Add Cover

  return <div className={`container ${mode === "light" ? "text-black" : "text-white"}`}>
      <section>
          <div className="display-4">{postData.title}</div>
          <div className="user-info d-flex">
              <img src="" alt="pfp" className="mx-2" width="32" height="32" />
              <div className="text-muted">Username</div>
          </div>
          <div className="text-muted">Date</div>
          <hr />
      </section>
      <div className="container" dangerouslySetInnerHTML={{__html: md.render(postData.body)}}>
      </div>
  </div>;
}